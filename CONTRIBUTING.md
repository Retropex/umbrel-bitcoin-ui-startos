...
The upstream UI source is vendored as a git submodule at [`umbrel-bitcoin/`](./umbrel-bitcoin) (the `startos` branch of [`Retropex/umbrel-bitcoin`](https://github.com/Retropex/umbrel-bitcoin)). Initialize it before the first build:

```bash
git submodule update --init --recursive   # fetch the vendored UI source
